...
class Kayttoliittyma:
    ...
    def kaynnista(self):
        ...
        summa_painike = ttk.Button(
            master=self._root,
            text="Summa",
            command=lambda: self._suorita_komento(Komento.SUMMA, int(self._syote_kentta.get()))
        )

        erotus_painike = ttk.Button(
            master=self._root,
            text="Erotus",
            command=lambda: self._suorita_komento(Komento.EROTUS, int(self._syote_kentta.get()))
        )
    ...
    def _suorita_komento(self, komento, syote = None):
        if komento == Komento.KUMOA:
            self._sovelluslogiikka.aseta_arvo(self._vanha_arvo)
        else:
            self._vanha_arvo = self._sovelluslogiikka._arvo
